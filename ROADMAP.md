...
- Extract roads, structures, walls, and boundaries. **Implemented; unit verified; UI wired (manual + AI-assisted).** `HistoricMapFeature` with typed geometry (`ROAD`, `STRUCTURE`, `WALL`, `BOUNDARY`) persists per map with confidence and notes; the historic map panel's feature bar lets a user trace a feature by tapping points on the map, confidence is scored against the real terrain relief via `MapTerrainAgreement.rasterizePolyline`/`.score` at save time (not fabricated), and saved features render as colored polylines. **Automatic extraction**: the "Auto-detect features (AI)" button sends the scanned map image to the configured cloud AI provider (`HistoricMapFeatureExtractor`), which traces visible roads/structures/walls/boundaries as normalized-image-coordinate polylines; proposals render on the map in a distinct color and are never written to the database until the user accepts each one individually (or dismisses it), matching the app's confirm-write rule — accepted proposals go through the same real georeferencing and terrain-agreement scoring as a manually traced feature.
