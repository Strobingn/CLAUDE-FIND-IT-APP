...
- Port chunk-seek **full-resolution** sparse preview (keep 1,024, fewer points) then exact fill  
- `putMemory` / `putDisk` split API if disk write ever blocks  
- Background upgrade of GPU mesh after intermediate publish without double full builds on cache hits  
